...
def generate_profile_card(
    ...
    if mini_scores:
        ...
        for game, val in mini_scores.items():
            g_name = game.replace("animequiz", "Quiz").replace("higherlower", "Higher/Lower").capitalize()
            draw.text((bar_x + 20, m_y_start + 30 + offset * 28), f"{g_name}", font=font_label, fill=(160, 200, 255))
            draw.text((bar_x + 250, m_y_start + 30 + offset * 28), str(val), font=font_value, fill=(255, 175, 200))
            offset += 1
    else:
        draw.text((bar_x, m_y_start), "Mini‑jeux", font=font_label, fill=(200, 200, 200))
