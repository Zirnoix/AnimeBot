...
class Stats(commands.Cog):
    ...
    async def monchart(self, ctx: commands.Context, username: str | None = None) -> None:
        ...
        try:
            buf = core.generate_genre_chart(chart_data, title=f"Genres préférés de {username}")
            file = discord.File(buf, filename="monchart.jpg")
            embed = discord.Embed(color=discord.Color.dark_green())
            embed.set_image(url="attachment://monchart.jpg")
            await ctx.send(embed=embed, file=file)
        except Exception:
            # Fallback to text chart if image fails
