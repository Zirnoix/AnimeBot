...
def _key(anime: Dict[str, Any], tag: str) -> str:
    # clé unique: titre_romaji|episode|tag(15/30)
...
def _should_alert(anime: Dict[str, Any], minutes_before: int) -> bool:
    ...
    if not airing:
        return False
    diff = airing - _now_ts()
    # on déclenche si dans la fenêtre [0, 60s] autour de la cible (tolérance)
...
class Alerts(commands.Cog):
    ...
    async def _get_alert_channel(self) -> Optional[discord.TextChannel]:
        cid = os.getenv("ALERT_CHANNEL_ID")
        if not cid:
            LOG.warning("ALERT_CHANNEL_ID manquant : aucune alerte envoyée.")
            return None
        try:
            ch = self.bot.get_channel(int(cid))
            if ch is None:
                # guilds pas encore en cache
                await self.bot.wait_until_ready()
                ch = self.bot.get_channel(int(cid))
            return ch if isinstance(ch, discord.TextChannel) else None
        except Exception as e:
            LOG.exception("ALERT_CHANNEL_ID invalide: %s", e)
            return None
    ...
    async def _users_next(self) -> List[Dict[str, Any]]:
        # parcourt tous les users liés si tu as core.load_links()
        ...
        for _uid, username in (links or {}).items():
            try:
                item = core.get_user_next_airing_one(username)  # à avoir dans core
                if item:
                    out.append(item)
            except Exception as e:
    ...
    async def _maybe_send(self, ch: discord.TextChannel, anime: Dict[str, Any], tag: str) -> None:
        ...
        if self.sent.get(k):
            return
        # petite carte texte simple (tu peux appeler generate_next_card si tu préfères une image)
    ...
    async def check_airing(self):
        ...
        # 1) Ton AniList (ANILIST_USERNAME)
